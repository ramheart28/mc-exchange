...
public class ChatRelayService {
    ...
    private static final String BACKEND_URL = System.getProperty("mcexchange.backend.url", "http://localhost:3000");
    private static final Gson gson = new Gson();
    
    public static void sendExchangeData(ExchangeParser.ExchangeData exchange) {
        ChatRelayMod.LOGGER.info("🚀 Attempting to send exchange data to backend: {} {} -> {} {}", 
            exchange.input_qty, exchange.input_item_id, exchange.output_qty, exchange.output_item_id);
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                ChatRelayMod.LOGGER.info("📡 Sending HTTP request to: {}", BACKEND_URL + "/api/exchanges");
                ChatRelayMod.LOGGER.info("📋 JSON payload: {}", json);
                
                client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                    .thenAccept(response -> {
                        if (response.statusCode() == 200 || response.statusCode() == 201) {
                            ChatRelayMod.LOGGER.info("Exchange data sent successfully! Status: {}, Response: {}", 
                                response.statusCode(), response.body());
                        } else {
                            ChatRelayMod.LOGGER.error("Failed to send exchange data: {} - {}", 
                                response.statusCode(), response.body());
                        }
                    })
                    .exceptionally(ex -> {
